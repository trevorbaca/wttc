...
def GLOBALS(skips):
    ...
    wrappers = baca.override.metronome_mark_extra_offset(skips[1 - 1], (0, 2))
    baca.tags.tag(wrappers, library.ONLY_LETTER_PARTS_GUITAR_1)
    wrappers = baca.override.metronome_mark_extra_offset(skips[1 - 1], (0, 2))
    baca.tags.tag(wrappers, library.ONLY_LETTER_PARTS_GUITAR_2)
...
def GT1(voice, meters):
    ...
    def block():
        ...
        library.annotate(pleaves[2:], 2)
        rmakers.attach_beams_to_runs_by_leaf_list([components[-1][:]])
...
def GT2(voice, meters):
    ...
    def block():
        ...
        library.annotate(pleaves[1:2], 1)
        library.annotate(pleaves[2:], 2)
        rmakers.attach_beams_to_runs_by_leaf_list([components[0][:]])
...
def fl(m):
    ...
    library.B1a_foo(library.pleaves(m[7, 8], 1), "Eb4", '"f"', stblf=True)
    library.B3(
...
def align_spanners(cache):
    ...
    baca.override.dls_staff_padding(gt2[14], 8)
    baca.override.dls_staff_padding(gt2[15], 5)
    ...
    baca.override.tuplet_bracket_staff_padding(gt2[14], 3.5)
    vn = cache["vn"]
...
def make_score(first_measure_number, previous_persistent_indicators):
    ...
    baca.section.set_up_score(
        ...
        measure_initial_grace_notes={
            3: ["%! +LETTER_PARTS_VIOLIN", r"\grace { s8 }"],
        },
...
def make_layout(environment):
    breaks = baca.layout.Breaks(
        baca.layout.Page(
            1,
            baca.layout.System(1, y_offset=10, distances=(12, 21, 21, 21, 21, 21)),
            baca.layout.System(6, y_offset=160, distances=(12, 21, 21, 21, 21, 21)),
        ),
        baca.layout.Page(
            2,
            baca.layout.System(12, y_offset=10, distances=(12, 21, 31, 21, 21, 21)),
        ),
    )
