...
def _reference_meters():
    ...
    strings = (
        ...
        "(8/4 ((4/4 (1/4 1/4 1/4 1/4)) (4/4 (1/4 1/4 1/4 1/4))))",
        "(9/4 (1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4))",
        "(10/4 (1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4))",
        "(11/4 (1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4))",
        "(12/4 (1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4 1/4))",
...
def B2a(pleaves, pitch, dynamics):
    ...
    for plt, dynamic in zip(plts, dynamics, strict=True):
        baca.dynamic(plt.get_head(), dynamic)
        baca.laissez_vibrer(plt.get_tail())
...
def C1a(pleaves, fundamental, harmonic_1, harmonic_2, dynamic):
    ...
    done_chord_1 = False
    for plt in plts:
        if isinstance(plt.get_head(), abjad.Chord):
            if not done_chord_1:
...
def C1c(pleaves, dyad, alteration, dynamics):
    ...
    for plt, dynamic in zip(plts, dynamics, strict=True):
        ...
        for chord in plt:
            baca.tweak.style_harmonic(target=chord.note_heads[1])
        baca.triple_staccato(plt.get_head())
        if dynamic != "-":
            baca.dynamic(plt.get_head(), dynamic)
        baca.spanners.trill(
            plt,
...
def G4a(pleaves, *, once=False, up_bow=False):
    ...
    for plt in plts:
        ...
        baca.stop_on_string(plt.get_head())
    if once is True:
        string = '"ff"'
    else:
        string = '"ff"-sempre'
...
def H1a(pleaves, pitches, dynamics):
    ...
    for plt, dynamic in zip(plts, dynamics, strict=True):
        baca.dynamic(plt.get_head(), dynamic)
        baca.stop_on_string(plt.get_tail())
...
def J3b(pleaves, pitches, dynamics, falls):
    ...
    for i, plt in enumerate(plts):
        if falls[i] == "0":
            baca.articulation(plt.get_head(), "bendAfter #'-4")
        else:
            baca.articulation(plt.get_head(), "bendAfter #'4")
...
def M5a(pleaves, pitches, falls, dynamics=None):
    ...
    for plt, fall, dynamic in zip(plts, falls, dynamics, strict=True):
        if fall == "0":
            baca.articulation(plt.get_tail(), "bendAfter #'-4")
        else:
            assert fall == "1"
            baca.articulation(plt.get_tail(), "bendAfter #'4")
        baca.dynamic(plt.get_head(), dynamic)
